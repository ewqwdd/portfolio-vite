...
export default function Navigation({ goTo, entity }: NavigationProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const index =
        links.length - linksCopy.findIndex((elem) => position >= elem.href) - 1;
      setGlow(index);
    };
    document
      .getElementById("root")
    ...
    <nav className="flex flex-col items-end mt-[5svh] gap-[10px] text-right">
      {links.map((elem, index) => (
        <span
          ...
          className={[
            "transition-all gap-4 inline-flex justify-end items-center",
            styles.link,
            index === glow ? styles.active : "",
          ].join(" ")}
        >
          {elem.label}
        </span>
